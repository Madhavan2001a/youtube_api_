...
background-image: url("https://images.unsplash.com/photo-1513077202514-c511b41bd4c7?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTd8fHlvdXR1YmUlMjBiYWNrZ3JvdW5kfGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60");
...
st.title(':silver[Youtube Data Scraping And Harvesting]')

#  function to connect to youtube api
@st.cache_data
def youtube_authenticate():
    return googleapiclient.discovery.build(
        "youtube", "v3", developerKey='AIzaSyDPvcRBsi-kf67ZNJMj6TiUPNKWsabZQjU')
    ...
    if st.button(":green[Scrape Data]"):
        ...
        if __name__ == "__main__":
            ...
            for option in options:
                ...
                SQL_channel_details_df = pd.concat(
                    [SQL_channel_details_df, channels_df[channels_df['channelTitle'] == option]])
                ...
                SQL_plalist_df = pd.concat([SQL_plalist_df, playlist_df[
                    playlist_df['channelId'] == (
                    channels_df[channels_df['channelTitle'] == option]['channelId'].values[0])]])
            ...
            if __name__ == "__main__":
                try:
                    ...
                    if mydb.is_connected():
                        ...
                        cursor.execute("select database();")
                        record = cursor.fetchone()
                        st.write(":green[Connected to MySQL Server version :]", db_Info, ":green[Database is :]",
                                 record)
                        ...
                        if SQL_question == '2. Which channels have the most number of videos, and how many videos do they have?':
                            cursor.execute(
                                "SELECT Channel_name, Number_of_Videos FROM channel_det ORDER BY Number_of_Videos DESC")
                            ans2 = cursor.fetchall()
                            df2 = pd.DataFrame(ans2, columns=['Channel_name', 'Number_of_Videos']).reset_index(
                                drop=True)
                            df2.index += 1
                            st.dataframe(df2)
                            st.write("Select next question and see the details")
